package com.pixelatedslice.easyconfig.api.config.node.factory.nodes.common.collection;

import com.google.common.reflect.TypeToken;
import com.pixelatedslice.easyconfig.api.config.node.env.EnvNode;
import com.pixelatedslice.easyconfig.api.config.node.factory.NodeBuilders;
import com.pixelatedslice.easyconfig.api.config.node.factory.builder.FactoryNodeBuilderEnvStep;
import com.pixelatedslice.easyconfig.api.config.node.factory.builder.FactoryNodeBuilderValueStep;
import com.pixelatedslice.easyconfig.api.config.node.factory.nodes.Nodes;
import com.pixelatedslice.easyconfig.api.config.node.value.ValueNode;
import com.pixelatedslice.easyconfig.api.validator.Validator;
import org.jspecify.annotations.Nullable;

import java.util.Collection;
import java.util.function.Function;

public interface CommonNodesCollection {
    static <T> TypeToken<T> newTypeToken() {
        return new TypeToken<T>() {
        };
    }

    default <T> FactoryNodeBuilderValueStep<Collection<T>> collectionValue(String key) {
        return Nodes.INSTANCE.value(CommonNodesCollection.<Collection<T>>newTypeToken()).key(key);
    }

    default <T> ValueNode<Collection<T>> emptyCollectionValue(String key) {
        return Nodes.INSTANCE.emptyValue(newTypeToken(), key);
    }

    default <T> ValueNode<Collection<T>> collectionValue(String key, @Nullable Collection<T> defaultValue) {
        return NodeBuilders.INSTANCE.createValueNode(newTypeToken(), key, defaultValue, null);
    }

    default <T> ValueNode<Collection<T>> collectionValue(String key, @Nullable Collection<T> defaultValue,
            @Nullable Collection<T> value) {
        return NodeBuilders.INSTANCE.createValueNode(newTypeToken(), key, defaultValue, value);
    }

    default <T> FactoryNodeBuilderEnvStep.VariableStep<Collection<T>> collectionEnv(String key) {
        return Nodes.INSTANCE.env(CommonNodesCollection.<Collection<T>>newTypeToken()).key(key);
    }

    default <T> EnvNode<Collection<T>> collectionEnv(String key, String variable) {
        return Nodes.INSTANCE.env(newTypeToken(), key, variable);
    }

    default <T> EnvNode<Collection<T>> collectionEnv(String key, String variable,
            @Nullable Function<String, @Nullable Collection<T>> adapter, @Nullable Validator<Collection<T>> validator) {
        return Nodes.INSTANCE.env(newTypeToken(), key, variable, adapter, validator);
    }
}
